...
class Live_score extends CI_Controller
{
	...
	function index()
	{
		...
		$bowler_innings = $this->live_score_model->getBowlerInnings($bowlerOverData);
	...
	function insertBallRecords()
	{
		...
		if ($data['ballnumber'] == 6 && !array_key_exists('wide', $data) && !array_key_exists('noball', $data)) {
	...
	function insertOverRecords($data)
	{
		...
		$bowler_innings = $this->live_score_model->getBowlerInnings($data);
	...
	function new_over()
	{
		...
		if ($totalOver['match_overs'] != $over['over_number']) {
			...
			$bowler_record = $this->live_score_model->get_bowler_score($match_id);
