...
class Livescore_display_model extends CI_Model
{
	...
	{
		$this->db->select('M.match_id,team_1,team_2,T.team_name as team1_name,T1.team_name as team2_name,runs');
		$this->db->join('teams as T', 'T.team_id = M.team_1');
		$this->db->join('teams as T1', 'T1.team_id = M.team_2');
		$this->db->join('match_logs as MM', 'MM.match_id = M.match_id');
		return $this->db->get_where('matches AS M', array('live_match' => 1))->row_array();
	}
